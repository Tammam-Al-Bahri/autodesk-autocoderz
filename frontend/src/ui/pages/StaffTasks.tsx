...
import { ClipboardList, History, CheckCircle2, Clock, User, AlertTriangle } from "lucide-react";
...
export default function StaffTasks({ ...props }: Props) {
    ...
    const fetchEverything = async (showLoading = true) => {
        ...
        try {
            ...
            let uRes = await apiFetch(apiUrl + `/users/me?_t=${Date.now()}`, {
                credentials: "include",
            }); // adding the timestamp here because the browser was caching the old user data and it was driving me mad lol
            if (!uRes.ok) return;
            ...
            let rRes = await apiFetch(apiUrl + `/rooms?_t=${Date.now()}`, {
                credentials: "include",
            });
            ...
            // get past tasks
            let hRes = await apiFetch(apiUrl + `/users/me/task-history?_t=${Date.now()}`, {
                credentials: "include",
            });
    ...
    return (
        <div className="max-w-md mx-auto mt-8 px-4 pb-20 bg-background text-foreground">
            <div className="text-4xl text-center font-black uppercase tracking-tighter leading-none text-foreground py-16">
                {props.buildingName}
            </div>
            {/* Header section */}
                ...
                {currentView === "pending" ? (
                    tasks.length === 0 ? (
                        <div className="text-center p-14 border-2 border-dashed border-border/60 dark:border-border/30 rounded-[2rem] bg-muted/10 dark:bg-muted/5">
                            ...
                            <p className="text-muted-foreground font-bold tracking-tight text-sm">
                        ...
                        tasks.map((t: any) => (
                            <Card
                                key={t.id}
                                className="border-none shadow-md bg-card dark:bg-card/95 overflow-hidden ring-1 ring-border/50 dark:ring-border/30 rounded-3xl group"
                            >
                                <div className="h-1.5 w-full bg-red-600 dark:bg-rose-600" />{" "}
                                {/* red strip at the top to make it look urgent */}
                                <CardContent className="p-6">
                                    <div className="flex justify-between items-start mb-5">
                                        <div>
                                            <span className="text-sm font-black text-muted-foreground uppercase tracking-widest mb-1 block">
                                                Location
                                            </span>
                                            <span className="text-3xl font-black leading-none block text-foreground">
                                                Room {t.number}
                                            </span>
                                        </div>
                                        <Badge
                                            variant="destructive"
                                            className="uppercase text-[9px] font-black tracking-widest px-3 py-1 flex items-center gap-1 shadow-sm dark:bg-red-900 dark:text-red-100"
                                        >
                    ...
                    <div className="text-center p-14 border-2 border-dashed border-border/60 dark:border-border/30 rounded-[2rem] bg-muted/10 dark:bg-muted/5">
                        {/* show this if they haven't actually finished any tasks yet */}{" "}
                    ...
                    historyLogs.map((item: any) => (
                        <Card
                            key={item.id}
                            className="border border-border/50 dark:border-border/30 shadow-sm bg-card/50 dark:bg-card/40 rounded-[2rem]"
                        >
                            <CardContent className="p-5">
                                <div className="flex justify-between items-center mb-4">
                                    ...
                                    <Badge
                                        variant="outline"
                                        className="text-[9px] font-black uppercase tracking-widest text-emerald-500 dark:text-emerald-400 border-emerald-500/30 dark:border-emerald-500/20 bg-emerald-500/10 dark:bg-emerald-500/5 px-3 py-1"
                                    >
                                ...
                                {item.message && (
                                    <div className="bg-background dark:bg-zinc-900/40 p-3.5 rounded-2xl border border-border/80 dark:border-border/40 relative mt-2">
                                        {/* the absolute label here was a bit of a pain to align but looks good */}{" "}
                                        <span className="absolute -top-2 left-3 bg-background dark:bg-zinc-900 px-1 text-[8px] font-black uppercase tracking-widest text-muted-foreground">
                                            Notes
                                        </span>
