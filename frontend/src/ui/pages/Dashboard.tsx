...
let getStatusStyle = (status: any) => {
    if (status === "ACTIVE")
        return { dot: "bg-green-500 dark:bg-green-400", bg: "bg-green-500/10 dark:bg-green-500/5" };
    if (status === "INACTIVE")
        return { dot: "bg-red-500 dark:bg-red-400", bg: "bg-red-500/10 dark:bg-red-500/5" };
    if (status === "DRAFT")
        return {
...
export default function Dashboard() {
    ...
    let currentBuilding = builds.find((b: Building) => b.id === bId);
                    ...
                    <>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                            ...
                            <StatCard
                                ...
                                icon={
                                    <Building2 className="w-5 h-5 text-primary dark:text-primary/80" />
                                }
                            />
                            ...
                            <StatCard
                                ...
                                icon={
                                    <AlertCircle className="w-5 h-5 text-destructive dark:text-rose-400" />
                                }
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            <Card className="lg:col-span-2 bg-primary dark:bg-primary/90 text-primary-foreground border-none rounded-3xl shadow-md">
                                <CardContent className="p-12 text-center">
                                    ...
                                    {currentBuilding.urn ? (
                                        <AutodeskViewer
                                            urn={currentBuilding.urn}
                                            token={autodeskToken}
                                        />
                                    ) : (
                                        <UploadBuildingModel buildingId={currentBuilding.id} />
                                    )}
                                </CardContent>
                            </Card>
...
function StatCard({
    ...
    value,
    icon,
    color = "bg-muted dark:bg-zinc-800/50",
    isSmall = false,
}: any) {
    return (
        <Card className="rounded-2xl border border-border/50 dark:border-border/30 shadow-sm bg-card dark:bg-zinc-900/95">
            <CardContent className="p-6">
                ...
                <p className={cn("font-black text-foreground", isSmall ? "text-sm" : "text-2xl")}>
                    {value}
                </p>
            </CardContent>
        </Card>
