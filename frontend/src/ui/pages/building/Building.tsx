...
export default function Building() {
    ...
    const fetchRooms = async (showLoad = true) => {
        ...
        try {
            ...
            let res = await apiFetch(
                apiUrl + "/rooms?buildingId=" + buildingId + "&_t=" + Date.now(),
                { credentials: "include" },
            );
    ...
    useEffect(() => {
        ...
        async function fetchData() {
            try {
                let buildingRes = await apiFetch(
                ...
                let tokenRes = await apiFetch(apiUrl + apsBase + apsRoutes.viewerToken, {
                    method: "GET",
                    credentials: "include",
                });
            ...
            <section className="space-y-4 pt-4 border-t border-border/60 dark:border-border/40">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                            Add and manage rooms so the receptionist can assign guests.
                        </p>
                    </div>
                    <button
                ...
                <div className="space-y-3 mt-4">
                    {roomsLoading && rooms.length === 0 ? (
                        <div className="text-center p-6 text-muted-foreground text-sm font-bold">
                            Loading rooms...
                        </div>
                    ) : rooms.length === 0 ? (
                        ...
                        rooms.map((room: any) => (
                            <Card
                                key={room.id}
                                className="border border-border/50 dark:border-border/40 shadow-sm bg-card dark:bg-zinc-900/80"
                            >
                                <CardContent className="p-4 flex justify-between items-center">
                                    {editingRoomId === room.id ? (
                                        <div className="flex gap-4 w-full items-center">
                                            ...
                                            <div className="flex gap-2 ml-auto">
                                                ...
                                                <Button
                                                    ...
                                                    variant="outline"
                                                    className="font-bold border-border/50 dark:border-border/40 hover:bg-muted dark:hover:bg-zinc-800 text-foreground"
                                                >
                                                    Cancel
                                                </Button>
                                            </div>
                                        ...
                                        <>
                                            <div className="flex items-center gap-4">
                                                <span className="text-xl font-black w-24 text-foreground">
                                                    Room {room.number}
                                                </span>
                                                <Badge
                                                    variant="outline"
                                                    className="font-bold uppercase text-muted-foreground border-border/50 dark:border-border/40 bg-muted/30 dark:bg-zinc-800/50"
                                                >
                                                    {room.type || "SINGLE"}
                                                </Badge>
                                                <Badge
                                                    className={
                                                        room.status === "Clean"
                                                            ? "bg-green-500 dark:bg-green-500/20 text-white dark:text-green-400 hover:bg-green-600"
                                                            : "bg-slate-500 dark:bg-slate-500/20 text-white dark:text-slate-300 hover:bg-slate-600"
                                                    }
                                                >
                                                    {room.status}
                                                </Badge>
                                            ...
                                            <div className="flex gap-2">
                                                <Button
                                                    ...
                                                    variant="outline"
                                                    size="sm"
                                                    className="font-bold border border-border/50 dark:border-border/40 bg-background dark:bg-zinc-950 hover:bg-muted dark:hover:bg-zinc-800 text-foreground"
                                                >
                                                    Edit
                                                </Button>
                                                <Button
                                                    ...
                                                    size="sm"
                                                    className="bg-rose-600 dark:bg-rose-600 hover:bg-rose-700 dark:hover:bg-rose-500 text-white font-bold shadow-sm"
                                                >
                                                    Delete
                                                </Button>
                                            </div>
