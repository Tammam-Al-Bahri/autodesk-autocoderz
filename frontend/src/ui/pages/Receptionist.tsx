...
let statuses: any = {
    Clean: {
        ...
        bg: "bg-emerald-500/10 border-emerald-500/30 dark:bg-emerald-500/10 dark:border-emerald-500/30",
        icon: Sparkles,
    },
    Dirty: {
        color: "text-rose-500 dark:text-rose-400",
        bg: "bg-rose-500/10 border-rose-500/30 dark:bg-rose-500/10 dark:border-rose-500/30",
        icon: BedDouble,
    },
    Occupied: {
        color: "text-blue-500 dark:text-blue-400",
        bg: "bg-blue-500/10 border-blue-500/30 dark:bg-blue-500/10 dark:border-blue-500/30",
        icon: Key,
    },
    Maintenance: {
        color: "text-amber-500 dark:text-amber-400",
        bg: "bg-amber-500/10 border-amber-500/30 dark:bg-amber-500/10 dark:border-amber-500/30",
        icon: Wrench,
    },
};
...
export default function Receptionist({ ...props }: Props) {
    ...
    useEffect(() => {
        ...
        const loadStuff = async () => {
            try {
                ...
                // fetch rooms
                let rRes = await apiFetch(apiUrl + "/rooms?buildingId=" + buildingId, {
                    credentials: "include",
                });
                ...
                // fetch staff
                let sRes = await apiFetch(apiUrl + "/users/buildings/" + buildingId + "/staff", {
                    credentials: "include",
                });
    ...
    const handleCheckIn = async () => {
        ...
        if (response.ok) {
            ...
            setRooms((prev: any) =>
                prev.map((r: any) =>
                    r.id === currentRoom.id
                        ? { ...r, status: "Occupied", guest: guestName, bookingId: data.data.id }
                        : r,
                ),
            );
            setGuestName("");
    ...
    const handleCheckOut = async () => {
        ...
        if (res.ok) {
            setRooms((prev: any) =>
                prev.map((r: any) =>
                    r.id === currentRoom.id
                        ? { ...r, status: "Dirty", guest: undefined, bookingId: undefined }
                        : r,
                ),
            );
    ...
    const assignTask = async (sId: string) => {
        ...
        if (res.ok) {
            setRooms((prev: any) =>
                prev.map((r: any) =>
                    r.id === currentRoom.id
                        ? { ...r, status: "Maintenance", assignedToName: worker.name }
                        : r,
                ),
            );
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10 border-b border-border dark:border-border/50 pb-8">
                <div className="space-y-1">
                    ...
                    <h1 className="text-4xl font-black uppercase tracking-tighter leading-none text-foreground">
                        <span className="text-accent-foreground">{props.buildingName} </span>
                        Reception
                    </h1>
                </div>
                ...
                <div className="flex gap-4">
                    ...
                    <div className="bg-emerald-500/10 dark:bg-emerald-900/20 border border-emerald-500/20 dark:border-emerald-800/30 px-4 py-2 rounded-xl text-center">
                        <p className="text-[9px] font-black uppercase tracking-widest text-emerald-500 dark:text-emerald-400">
                            Ready
                        </p>
                        ...
                        return (
                            <div
                                ...
                                className={cn(
                                    ...
                                    conf.bg,
                                    isSelected
                                        ? "ring-4 ring-primary dark:ring-primary/80 ring-offset-4 ring-offset-background dark:ring-offset-zinc-950 scale-105 border-transparent shadow-lg"
                                        : "hover:scale-[1.02] hover:shadow-md",
                                )}
                            >
                                <Icon
                                    className={cn(
                                        "absolute top-3 right-3 w-4 h-4 opacity-50",
                                        conf.color,
                                    )}
                                />
                                <span
                                ...
                                <span
                                    className={cn(
                                        "text-[9px] uppercase font-black tracking-widest px-2 py-0.5 rounded-full bg-background/50 dark:bg-zinc-900/80",
                                        conf.color,
                                    )}
                                >
                                    {rm.status}
                ...
                <div className="space-y-4 sticky top-6">
                    {!currentRoom ? (
                        <Card className="h-64 flex flex-col items-center justify-center text-muted-foreground border-dashed border-2 dark:border-border/40 rounded-[2rem] bg-muted/10 dark:bg-muted/5">
                            ...
                            <p className="font-bold tracking-tight text-sm text-muted-foreground">
                        ...
                        <Card className="border-none shadow-xl bg-card dark:bg-zinc-900/95 rounded-[2rem] overflow-hidden ring-1 ring-border/50 dark:ring-border/40">
                            <div
                                className={cn(
                                    "h-2 w-full",
                                    statuses[currentRoom.status].bg
                                        .split(" ")[0]
                                        .replace("/10", ""),
                                )}
                            />
                            ...
                            <CardHeader className="border-b border-border/50 dark:border-border/40 pb-5 pt-6 px-8">
                                <div className="flex justify-between items-center">
                                    ...
                                    <Badge
                                        className={cn(
                                            "uppercase text-[10px] font-black px-3 py-1.5 border-none",
                                            statuses[currentRoom.status].bg,
                                            statuses[currentRoom.status].color,
                                        )}
                                    >
                                        {currentRoom.status}
                                    </Badge>
                                ...
                                {currentRoom.status === "Clean" && (
                                    <div className="space-y-5 animate-in fade-in slide-in-from-right-4 duration-300">
                                        ...
                                        <Button
                                            ...
                                            disabled={isLoading}
                                            className="w-full h-12 rounded-xl font-black uppercase text-[11px] tracking-widest shadow-lg shadow-primary/20 dark:shadow-primary/10 hover:scale-[1.02] transition-transform"
                                        >
                                ...
                                {currentRoom.status === "Occupied" && (
                                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                                        ...
                                        <Button
                                            ...
                                            disabled={isLoading}
                                            variant="destructive"
                                            className="w-full h-12 rounded-xl font-black uppercase text-[11px] tracking-widest shadow-lg shadow-rose-500/20 dark:shadow-rose-900/20 hover:scale-[1.02] transition-transform"
                                        >
                                ...
                                {currentRoom.status === "Maintenance" && (
                                    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
                                        ...
                                        <div className="space-y-2 pt-4 border-t border-border/50 dark:border-border/40">
                                            ...
                                            >
                                                <option value="" disabled>
                                                    Select Alternate Staff...
                                                </option>
                                                {staff
                                                    .filter(
                                                        (s: any) =>
                                                            s.name !== currentRoom.assignedToName,
                                                    )
                                                    .map((s: any) => (
                                                        <option key={s.id} value={s.id}>
                                                            {s.name}
